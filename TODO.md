...
- [x] 削除ボタンのレイアウト修正（時間表示の横に移動）
- [x] JWT有効期限延長（24h → 7d）
- [x] JWT を HttpOnly Cookie に移行（XSS対策）
- [x] リンク自動検出（URLをクリック可能に）
...
- [ ] テストコード追加
- [ ] パフォーマンス最適化
- [ ] `/api/auth/me` エンドポイント追加（認証チェックの改善）
- [ ] linkifyライブラリ導入検討（自前実装からの移行）
- [ ] 定期バックアップ（Cron Trigger + R2）
- [ ] JSONエクスポートAPI（`GET /api/export/posts`）
